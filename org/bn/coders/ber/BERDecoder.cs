		...
		public override DecodedObject<object> decodeSequenceOf(DecodedObject<object> decodedTag, System.Type objectClass, ElementInfo elementInfo, System.IO.Stream stream)
		{
			...
			if (len.Value != 0)
			{
				...
				do 
				{
                    ...
                    // Pavel
                    if (elementInfo.hasPreparedInfo() && elementInfo.PreparedInfo.hasASN1ElementInfo() && !elementInfo.PreparedInfo.ASN1ElementInfo.IsImplicitTag && first)
                    {
                        // Explicit tag, read 
                        DecodedObject<object> tmpTag = decodeTag(stream);
                        tmpLen = new DecodedObject<int>(len.Value, len.Size);
                        len = decodeLength(stream);
                        first = false;
                    }
                    // Pavel
